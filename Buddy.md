...
Пример(рис.): **выделить(256)->выделить(128)->выделить(64)**
...
![](/%D0%A0%D0%B8%D1%81%D1%83%D0%BD%D0%BE%D0%BA9.png)
...
На рис. представлен алгоритм в представлении бинарного дерева. Листья представляют текущее распределение памяти. Если два двойника являются листьями, то по крайней мере один из них занят; в противном случае они должны слиться в блок большего размера.
...
Каждый блок описывается с помощью структуры типа slab, которая является его дескриптором.
```
struct slab {
